...
function slugBreadcrumbLabel(slug: string): string {
  ...
  const last = path.split("/").pop() ?? "technology";
...
export interface TechnologyCapabilityItem {
  label: string;
  description?: string;
  badge?: string;
  icon?: string;
...
export interface TechnologyCapabilitiesProps {
  id: string;
  eyebrow: string;
  title: string;
  intro: string;
  items: TechnologyCapabilityItem[];
...
export interface TechnologyProcessProps {
  id: string;
  eyebrow: string;
  title: string;
  intro: string;
...
export interface TechnologyUseCaseItem {
  label: string;
  description?: string;
  audience?: string;
  badge?: string;
...
export interface TechnologyUseCasesProps {
  id: string;
  eyebrow: string;
  title: string;
  intro: string;
  items: TechnologyUseCaseItem[];
  cta?: { label: string; href: string };
}
...
export interface TechnologyTechStackProps {
  id: string;
  eyebrow: string;
  title: string;
  intro: string;
...
const DEFAULT_CAPABILITIES: TechnologyCapabilityItem[] = [
  {
...
const DEFAULT_PROCESS_STEPS: TechnologyProcessStep[] = [
  {
    step: 1,
...
const DEFAULT_USE_CASES: TechnologyUseCaseItem[] = [
  {
...
const DEFAULT_STACK_CATEGORIES: TechnologyStackCategory[] = [
  {
    name: "Backend & APIs",
...
export function mapTechnologyBreadcrumbs(entity: ConfigEntity): BreadcrumbItem[] {
  const hero = asRecord(entity.hero);
  const label = (hero.breadcrumb_label as string) ?? slugBreadcrumbLabel(entity.slug);
  return [{ label: "Home", href: "/" }, { label: "Technologies", href: "/technologies/" }, { label }];
}

export function mapTechnologyHero(entity: ConfigEntity): TechnologyHeroProps {
  const hero = asRecord(entity.hero);
  ...
  const secondaryHref = (hero.secondary_cta_href as string) ?? "#tech-categories";

  return {
    breadcrumbLabel: (hero.breadcrumb_label as string) ?? slugBreadcrumbLabel(entity.slug),
    kickerPrefix: (hero.kicker_prefix as string) ?? "Technologies",
    ...
    title:
      ...
      `Web, Mobile & SaaS development tech stack we use at QalbIT.`,
    intro:
    ...
    primaryCta: {
      label: (hero.primary_cta_label as string) ?? "Discuss your tech stack",
      href: primaryHref,
    },
    secondaryCta: {
      label: (hero.secondary_cta_label as string) ?? "View technologies by category",
      href: secondaryHref,
      external: secondaryHref.startsWith("http"),
    },
    snapshotTitle: (hero.snapshot_title as string) ?? "Tech stack snapshot",
    snapshot:
      snapshot.length > 0
        ? snapshot
        : [
...
export function mapTechnologyOverview(entity: ConfigEntity): TechnologyOverviewProps {
  ...
  return {
    id: (overview.id as string) ?? "technology-overview",
    ...
    leftTitle: (overview.left_title as string) ?? "Where this technology fits best",
...
export function mapTechnologyCapabilities(entity: ConfigEntity): TechnologyCapabilitiesProps {
  const cap = asRecord(entity.capabilities);
  const rawItems = asArray<JsonRecord>(cap.items);

  const items: TechnologyCapabilityItem[] =
    rawItems.length > 0
      ? rawItems.map((item) => ({
  ...
  return {
    id: (cap.id as string) ?? "technology-capabilities",
...
export function mapTechnologyProcess(entity: ConfigEntity): TechnologyProcessProps {
  const process = asRecord(entity.process);
  const rawSteps = asArray<JsonRecord>(process.items ?? process.steps);

  const steps: TechnologyProcessStep[] =
    rawSteps.length > 0
  ...
  return {
    id: (process.id as string) ?? "technology-process",
    eyebrow: (process.eyebrow as string) ?? "How we typically run this technology",
...
export function mapTechnologyUseCases(entity: ConfigEntity): TechnologyUseCasesProps {
  const useCases = asRecord(entity.use_cases);
  const rawItems = asArray<JsonRecord>(useCases.items);

  const items: TechnologyUseCaseItem[] =
    rawItems.length > 0
      ? rawItems.map((item) => {
  ...
  return {
    id: (useCases.id as string) ?? "technology-use-cases",
    eyebrow: (useCases.eyebrow as string) ?? "Where this technology fits best",
    title: (useCases.title as string) ?? `When to choose ${entity.name} with QalbIT`,
    intro:
      (useCases.intro as string) ??
      "Here are typical scenarios where this technology is the right fit, based on projects we run most often.",
    items,
    cta: mapCtaLink(asRecord(useCases.cta)),
  };
}

export function mapTechnologyTechStack(entity: ConfigEntity): TechnologyTechStackProps {
  const stack = asRecord(entity.stack);
  const rawCategories = asArray<JsonRecord>(stack.categories);

  const categories: TechnologyStackCategory[] =
  ...
  return {
    id: (stack.id as string) ?? "technology-tech-stack",
...
export function mapTechnologyCta(entity: ConfigEntity): TechnologyCtaProps | null {
...
export function mapTechnologyFaq(entity: ConfigEntity, faqs: FaqItem[]): FaqBlockProps | null {
