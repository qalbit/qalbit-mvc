<?php
/**
 * Industries Hero (Base Industries Page)
 *
 * Optional:
 * - $hero (array) to override defaults:
 *      [
 *          'breadcrumb_label'     => 'Industries',
 *          'kicker_prefix'        => 'Industries',
 ...
 *          'intro'                => 'QalbIT focuses on a practical, well-supported set of industries...',
 *          'primary_cta_label'    => 'Discuss your tech stack',
 *          'primary_cta_href'     => '/contact-us/',
 *          'secondary_cta_label'  => 'View industries by category',
...
// Basic industry-level fields
$industryName = $industry['name'] ?? 'Industry';
$industrySlug = $industry['slug'] ?? '/industries/';

// Derive last segment from slug for breadcrumb if label not provided
$slugPath    = trim((string) $industrySlug, '/');
$slugLast    = $slugPath ? basename($slugPath) : 'industry';
$defaultCrumbLabel = ucwords(str_replace('-', ' ', $slugLast));

$heroConfig = $industry['hero'] ?? [];

// Breadcrumb
$breadcrumbLabel = $heroConfig['breadcrumb_label'] ?? $defaultCrumbLabel;

// Kicker / pill
$kickerPrefix = $heroConfig['kicker_prefix'] ?? 'Industries';
...
$intro = $heroConfig['intro']
    ?? 'QalbIT focuses on a practical, well-supported set of industries – from React and Node.js to Laravel, Flutter and WordPress – so your product is easier to scale, maintain and hand over to future teams.';
...
$secondaryCtaLabel = $heroConfig['secondary_cta_label'] ?? 'View industries by category';
