<?php
/**
 * Service CTA section (S8)
 *
 * Expects:
 * - array $service with optional 'cta' config
 */

$service = $service ?? [];
$cta     = $service['cta'] ?? [];

// Allow turning CTA off per service
$ctaEnabled = $cta['enabled'] ?? true;
if (!$ctaEnabled) {
    return;
}

$serviceName = $service['name'] ?? 'software development';
